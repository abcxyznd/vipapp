// api/sync-ipa.js - BẢO MẬT URL + AUTO TAG V3 PRO + FIX LARGE FILE
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Thay vì fetch trực tiếp file (dễ lỗi 500 với file lớn), ta dùng chiến thuật:
    // A. Lấy SHA từ API thư mục (nhẹ)
    // B. Lấy nội dung từ Raw URL (không giới hạn dung lượng)
    // ==================================================================================
    console.log('📄 Fetching from GitHub (Large File Mode)...');
    
    // Đường dẫn API lấy danh sách file trong thư mục 'public/data'
    const getDirUrl = `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/public/data`;
    // Đường dẫn Raw để lấy nội dung text thuần
    ...
    try {
      ...
      // A. Xử lý SHA (Tìm file trong danh sách thư mục)
      if (dirResponse.ok) {
        const dirFiles = await dirResponse.json();
        // Tìm file ipa.json trong danh sách file trả về
        ...
        if (fileNode) {
         ...
         console.log('⚠️ Không lấy được thông tin thư mục (có thể chưa có folder data).');
      }

      // B. Xử lý Nội dung (Parse từ Raw Text)
      if (contentResponse.ok) {
        const rawText = await contentResponse.text();
        if (rawText && rawText.trim()) {
           try {
             currentData = JSON.parse(rawText);
             console.log(`✅ Current Data: ${currentData.length} apps (Loaded via Raw)`);
           } catch (parseError) {
             console.error('⚠️ File Raw bị lỗi JSON, sẽ reset:', parseError.message);
             currentData = [];
           }
        } else {
             console.log('⚠️ File Raw rỗng.');
             currentData = [];
        }
      } else {
         console.log('⚠️ Không tải được nội dung Raw (có thể file chưa tồn tại).');
         currentData = [];
      }

    } catch (githubError) {
      console.error('❌ GitHub fetch error:', githubError.message);
      // Không throw error, để code tiếp tục chạy và tạo file mới nếu cần
      currentData = [];
    }
